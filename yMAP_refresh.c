...
ymap__change            (char *a_func, char a_axis, char a_level)
{
   ...
   tyMAP      *x_map       = NULL;
   /*---(header)-------------------------*/
   DEBUG_YMAP_G  yLOG_enter   (a_func);
   /*---(defense)------------------------*/
   DEBUG_YMAP_G  yLOG_char    ("a_axis"    , a_axis);
   rc = ymap_pick_map (a_axis, &x_map, NULL);
   DEBUG_YMAP_G  yLOG_value   ("pickmap"   , rc);
   --rce;  if (rc < 0) {
      DEBUG_YMAP_G   yLOG_exitr   (a_func, rce);
      return rce;
   }
   DEBUG_YMAP_G  yLOG_point   ("x_map"     , x_map);
   /*---(make change)--------------------*/
   DEBUG_YMAP_G   yLOG_value   ("a_level"   , a_level);
   DEBUG_YMAP_G   yLOG_value   ("before"    , x_map->change);
   switch (a_level) {
   case YMAP_FULL :
   ...
   case YMAP_SRC  :
      ...
      break;
   default :
      DEBUG_YMAP_G   yLOG_note    ("unknown level");
      DEBUG_YMAP_G   yLOG_exitr   (a_func, rce);
      return rce;
   }
   DEBUG_YMAP_G   yLOG_value   ("after"     , x_map->change);
   /*---(complete)-----------------------*/
   DEBUG_YMAP_G  yLOG_exit    (a_func);
   return 0;
}
...
ymap_wide               (ushort a_pos, uchar a_wide)
{
   ...
   char        rc          =    0;
   /*---(header)-------------------------*/
   DEBUG_YMAP_G   yLOG_enter   (__FUNCTION__);
   DEBUG_YMAP_G   yLOG_value   ("a_pos"     , a_pos);
   DEBUG_YMAP_G   yLOG_value   ("gmax"      , g_xmap.gmax);
   /*---(defense)------------------------*/
   --rce;  if (a_pos > g_xmap.gmax) {
      DEBUG_YMAP_G   yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   /*---(update entry)-------------------*/
   DEBUG_YMAP_G   yLOG_value   ("a_wide"    , a_wide);
   rc = yMAP_entry (YMAP_XAXIS, a_pos, YMAP_LEAVE, a_wide, YMAP_CURR);
   DEBUG_YMAP_G   yLOG_value   ("entry"     , rc);
   /*---(update refresh)-----------------*/
   if (rc >= 0)   ymap_xchange (YMAP_POS);
   DEBUG_YMAP_G   yLOG_value   ("change"    , rc);
   /*---(complete)-----------------------*/
   DEBUG_YMAP_G   yLOG_exitr   (__FUNCTION__, rc);
   return rc;
}
char
ymap_tall               (ushort a_pos, uchar a_tall)
{
   ...
   DEBUG_YMAP_G   yLOG_enter   (__FUNCTION__);
   DEBUG_YMAP_G   yLOG_value   ("a_pos"     , a_pos);
   DEBUG_YMAP_G   yLOG_value   ("gmax"      , g_ymap.gmax);
   /*---(defense)------------------------*/
   ...
   /*---(update entry)-------------------*/
   DEBUG_YMAP_G   yLOG_value   ("a_tall"    , a_tall);
...
ymap_deep               (ushort a_pos, uchar a_deep)
{
   ...
   DEBUG_YMAP_G   yLOG_enter   (__FUNCTION__);
   DEBUG_YMAP_G   yLOG_value   ("a_pos"     , a_pos);
   DEBUG_YMAP_G   yLOG_value   ("gmax"      , g_zmap.gmax);
   /*---(defense)------------------------*/
   ...
   /*---(update entry)-------------------*/
   DEBUG_YMAP_G   yLOG_value   ("a_deep"    , a_deep);
...
ymap__refresh_analyze   (void)
{
   ...
   if (u_last == YMAP_EMPTY) {
      DEBUG_YMAP_G   yLOG_complex ("UNIV"      , "%4d, ····l, %4db, %4dc, %4de", rc, b, c, e);
   } else {
      DEBUG_YMAP_G   yLOG_complex ("UNIV"      , "%4d, %4dl, %4db, %4dc, %4de", rc, u_last, b, c, e);
   }
   if (c == u_last) {
      DEBUG_YMAP_G   yLOG_note    ("univ/tab did not change, do not remap");
   } else {
      DEBUG_YMAP_G   yLOG_note    ("univ/tab change, remap");
   ...
   if (x_last == YMAP_EMPTY) {
      DEBUG_YMAP_G   yLOG_complex ("COLS"      , "%4d, ····l, %4db, %4dc, %4de", rc, b, c, e);
   } else {
      DEBUG_YMAP_G   yLOG_complex ("COLS"      , "%4d, %4dl, %4db, %4dc, %4de", rc, x_last, b, c, e);
   }
   if (c == x_last) {
      DEBUG_YMAP_G   yLOG_note    ("xpos/col did not change, do not remap");
   } else {
      DEBUG_YMAP_G   yLOG_note    ("xpos/col change, remap");
   ...
   if (y_last == YMAP_EMPTY) {
      DEBUG_YMAP_G   yLOG_complex ("ROWS"      , "%4d, ····l, %4db, %4dc, %4de", rc, b, c, e);
   } else  {
      DEBUG_YMAP_G   yLOG_complex ("ROWS"      , "%4d, %4dl, %4db, %4dc, %4de", rc, y_last, b, c, e);
   }
   if (c == y_last) {
      DEBUG_YMAP_G   yLOG_note    ("ypos/row did not change, do not remap");
   } else {
      DEBUG_YMAP_G   yLOG_note    ("ypos/row change, remap");
      ymap_xchange (YMAP_FULL);   /* opposite axis */
   ...
   if (y_last == YMAP_EMPTY) {
      DEBUG_YMAP_G   yLOG_complex ("DEEP"      , "%4d, ····l, %4db, %4dc, %4de", rc, b, c, e);
   } else {
      DEBUG_YMAP_G   yLOG_complex ("DEEP"      , "%4d, %4dl, %4db, %4dc, %4de", rc, z_last, b, c, e);
   }
   if (c == z_last) {
      DEBUG_YMAP_G   yLOG_note    ("zpos/deep did not change, do not remap");
   } else {
      DEBUG_YMAP_G   yLOG_note    ("zpos/deep change, remap");
      ymap_xchange (YMAP_FULL);   /* opposite axis */
      ymap_ychange (YMAP_FULL);   /* opposite axis */
   ...
   z_last = c;
   /*---(ocmplete)-----------------------*/
   DEBUG_YMAP_G   yLOG_exit    (__FUNCTION__);
   return 0;
}
...
ymap__refresh           (char *a_name, char a_axis, tyMAP *a_map)
{
   ...
   /*---(header)-------------------------*/
   DEBUG_YMAP_G  yLOG_enter   (__FUNCTION__);
   DEBUG_YMAP_G  yLOG_note    (a_name);
   switch (a_map->change) {
   case YMAP_FULL :  /* call external mapper */
      /*---(get sizes)----------------------*/
      DEBUG_YMAP_G  yLOG_point   ("e_sizer"   , myMAP.e_sizer);
      if (myMAP.e_sizer == NULL) {
         DEBUG_YMAP_G  yLOG_note    ("without e_sizer, nothing to do");
         DEBUG_YMAP_G  yLOG_exitr   (__FUNCTION__, rce);
         return rce;
      }
      rc = myMAP.e_sizer (a_axis, &x_max, NULL, NULL, NULL, NULL, NULL, NULL);
      DEBUG_YMAP_G  yLOG_value   ("e_sizer"   , rc);
      --rce;  if (rc < 0) {
         DEBUG_YMAP_G  yLOG_exitr   (__FUNCTION__, rce);
         return rce;
      }
      DEBUG_YMAP_G  yLOG_value   ("x_max"     , x_max);
      --rce;  if (x_max <= 0) {
         DEBUG_YMAP_G  yLOG_exitr   (__FUNCTION__, rce);
         return rce;
      }
      ...
      for (i = 0; i < x_max; ++i) {
         ...
         DEBUG_YMAP_G  yLOG_value   ("append"    , rc);
         if (rc < 0)  break;
      }
      rc = yMAP_finish ();
   case YMAP_POS  :  /* process map and make display */
      DEBUG_YMAP_G  yLOG_complex ("CUR"       ,  "%4du, %4dg", g_umap.ucur, g_umap.gcur);
      rc = ymap_update_large (a_axis);
      DEBUG_YMAP_G  yLOG_value   ("update"    , rc);
      DEBUG_YMAP_G  yLOG_complex ("CUR"       ,  "%4du, %4dg", g_umap.ucur, g_umap.gcur);
   case YMAP_SRC  :  /* handle local ends */
      rc = ymap_update_small (a_axis);
      DEBUG_YMAP_G  yLOG_value   ("ends"      , rc);
      DEBUG_YMAP_G  yLOG_complex ("CUR"       ,  "%4du, %4dg", g_umap.ucur, g_umap.gcur);
      if (a_map->grid != NULL) {
         rc = myMAP.e_placer (a_axis, a_map->gbeg, a_map->gcur, a_map->gend);
         DEBUG_YMAP_G  yLOG_value   ("placer"    , rc);
      } else {
         DEBUG_YMAP_G  yLOG_note    ("grid is null, skipping placer");
      }
      DEBUG_YMAP_G  yLOG_complex ("CUR"       ,  "%4du, %4dg", g_umap.ucur, g_umap.gcur);
      break;
   }
...
yMAP_refresh            (void)
{
   ...
   --rce;  if (!yMODE_operational (MODE_MAP)) {
      DEBUG_YMAP_G  yLOG_note    ("can not execute until operational");
      DEBUG_YMAP_G  yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   /*---(look for combinations)----------*/
   DEBUG_YMAP_G   yLOG_complex ("change bef", "%du, %dx, %dy, %dz", g_umap.change, g_xmap.change, g_ymap.change, g_zmap.change);
   ymap__refresh_analyze ();
   DEBUG_YMAP_G   yLOG_complex ("change aft", "%du, %dx, %dy, %dz", g_umap.change, g_xmap.change, g_ymap.change, g_zmap.change);
   /*---(refresh axis)-------------------*/
   DEBUG_YMAP_G  yLOG_complex ("CUR"       ,  "%4du, %4dg", g_umap.ucur, g_umap.gcur);
   ...
   ymap_visu_update ();
   /*---(finalize)-----------------------*/
   DEBUG_YMAP_G  yLOG_point   ("e_done"    , myMAP.e_done);
   if (myMAP.e_done != NULL)  rc = myMAP.e_done ();
   DEBUG_YMAP_G  yLOG_value   ("e_done"    , rc);
   --rce;  if (rc < 0) {
      DEBUG_YMAP_G  yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
