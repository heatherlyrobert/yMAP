/*============================---beg-of-source---============================*/
/*---(headers)---------------------------*/
#include    "yMAP.h"
#include    "yMAP_priv.h"




char        g_print     [LEN_RECD] = "";



char
ymap_grid_clear         (tGRID *a_grid, ushort a_len)
{
   int         i           =    0;
   if (a_grid == NULL)  return -1;
   for (i = 0; i < a_len; ++i) {
      a_grid [i].ref  = YMAP_EMPTY;
      a_grid [i].wide = 0;
      a_grid [i].used = YMAP_NADA;
      a_grid [i].unit = 0;
   }
   return 0;
}

char
ymap_grid_set           (uchar a_axis, tyMAP *a_map, tGRID *a_grid)
{
   /*---(locals)-----------+-----+-----+-*/
   char        rce         =  -10;
   /*---(header)-------------------------*/
   DEBUG_MAP   yLOG_senter  (__FUNCTION__);
   /*---(map)----------------------------*/
   DEBUG_MAP   yLOG_schar   (a_axis);
   DEBUG_MAP   yLOG_spoint  (a_grid);
   --rce;  switch (a_axis) {
   case  YMAP_UNIV  :
      DEBUG_MAP    yLOG_snote   ("store in b");
      DEBUG_MAP    yLOG_spoint  (g_ugrid);
      if (g_ugrid != NULL)  ymap_grid_free ('-', a_axis);
      g_ugrid     = a_grid;
      a_map->grid = a_grid;
      DEBUG_MAP    yLOG_spoint  (g_ugrid);
      break;
   case  YMAP_XAXIS  :
      DEBUG_MAP    yLOG_snote   ("store in x");
      DEBUG_MAP    yLOG_spoint  (g_xgrid);
      if (g_xgrid != NULL)  ymap_grid_free ('-', a_axis);
      g_xgrid     = a_grid;
      a_map->grid = a_grid;
      DEBUG_MAP    yLOG_spoint  (g_xgrid);
      break;
   case  YMAP_YAXIS  :
      DEBUG_MAP    yLOG_snote   ("store in y");
      DEBUG_MAP    yLOG_spoint  (g_ygrid);
      if (g_ygrid != NULL)  ymap_grid_free ('-', a_axis);
      g_ygrid     = a_grid;
      a_map->grid = a_grid;
      DEBUG_MAP    yLOG_spoint  (g_ygrid);
      break;
   case  YMAP_ZAXIS  :
      DEBUG_MAP    yLOG_snote   ("store in z");
      DEBUG_MAP    yLOG_spoint  (g_zgrid);
      if (g_zgrid != NULL)  ymap_grid_free ('-', a_axis);
      g_zgrid     = a_grid;
      a_map->grid = a_grid;
      DEBUG_MAP    yLOG_spoint  (g_zgrid);
      break;
   case  YMAP_WHEN  :
      DEBUG_MAP    yLOG_snote   ("store in w");
      DEBUG_MAP    yLOG_spoint  (g_wgrid);
      if (g_wgrid != NULL)  ymap_grid_free ('-', a_axis);
      g_wgrid     = a_grid;
      a_map->grid = a_grid;
      DEBUG_MAP    yLOG_spoint  (g_wgrid);
      break;
   default  :
      DEBUG_MAP   yLOG_sexitr  (__FUNCTION__, rce);
      return rce;
   }
   /*---(complete)-----------------------*/
   DEBUG_MAP   yLOG_sexit   (__FUNCTION__);
   return 0;
}

char
ymap_grid_free          (uchar a_full, uchar a_axis)
{
   /*---(locals)-----------+-----+-----+-*/
   char        rce         =  -10;
   /*---(header)-------------------------*/
   DEBUG_MAP   yLOG_senter  (__FUNCTION__);
   /*---(map)----------------------------*/
   DEBUG_MAP   yLOG_schar   (a_axis);
   switch (a_axis) {
   case YMAP_UNIV :
      DEBUG_MAP    yLOG_snote   ("free b");
      DEBUG_MAP    yLOG_spoint  (g_ugrid);
      if (a_full != 'I' && g_ugrid != NULL) {
         /*> printf ("freeing grid\n");                                               <*/
         DEBUG_MAP   yLOG_snote   ("freeing");
         free (g_ugrid);
      }
      g_umap.grid = g_ugrid = NULL;
      DEBUG_MAP    yLOG_spoint  (g_ugrid);
      break;
   case YMAP_XAXIS :
      DEBUG_MAP    yLOG_snote   ("free x");
      DEBUG_MAP    yLOG_spoint  (g_xgrid);
      if (a_full != 'I' && g_xgrid != NULL) {
         /*> printf ("freeing grid\n");                                               <*/
         DEBUG_MAP   yLOG_snote   ("freeing");
         free (g_xgrid);
      }
      g_xmap.grid = g_xgrid = NULL;
      DEBUG_MAP    yLOG_spoint  (g_xgrid);
      break;
   case YMAP_YAXIS :
      DEBUG_MAP    yLOG_snote   ("free y");
      DEBUG_MAP    yLOG_spoint  (g_ygrid);
      if (a_full != 'I' && g_ygrid != NULL) {
         /*> printf ("freeing grid\n");                                               <*/
         DEBUG_MAP   yLOG_snote   ("freeing");
         free (g_ygrid);
      }
      g_ymap.grid = g_ygrid = NULL;
      DEBUG_MAP    yLOG_spoint  (g_ygrid);
      break;
   case YMAP_ZAXIS :
      DEBUG_MAP    yLOG_snote   ("free z");
      DEBUG_MAP    yLOG_spoint  (g_zgrid);
      if (a_full != 'I' && g_zgrid != NULL) {
         /*> printf ("freeing grid\n");                                               <*/
         DEBUG_MAP   yLOG_snote   ("freeing");
         free (g_zgrid);
      }
      g_zmap.grid = g_zgrid = NULL;
      DEBUG_MAP    yLOG_spoint  (g_zgrid);
      break;
   case YMAP_WHEN :
      DEBUG_MAP    yLOG_snote   ("free w");
      DEBUG_MAP    yLOG_spoint  (g_wgrid);
      if (a_full != 'I' && g_wgrid != NULL) {
         /*> printf ("freeing grid\n");                                               <*/
         DEBUG_MAP   yLOG_snote   ("freeing");
         free (g_wgrid);
      }
      g_wmap.grid = g_wgrid = NULL;
      DEBUG_MAP    yLOG_spoint  (g_wgrid);
      break;
   default  :
      DEBUG_MAP   yLOG_sexitr  (__FUNCTION__, rce);
      return rce;
   }
   /*---(complete)-----------------------*/
   DEBUG_MAP   yLOG_sexit   (__FUNCTION__);
   return 0;
}

char
ymap__clear             (uchar a_full, uchar a_axis)
{
   /*---(locals)-----------+-----+-----+-*/
   char        rce         =  -10;
   char        rc          =    0;
   tyMAP      *x_map       = NULL;
   tGRID      *x_grid      = NULL;
   int         i           =    0;
   /*---(header)-------------------------*/
   DEBUG_MAP   yLOG_enter   (__FUNCTION__);
   /*---(defense)------------------------*/
   DEBUG_MAP   yLOG_char    ("a_full"    , a_full);
   --rce;  if (a_full == 0 || strchr ("Iy-", a_full) == NULL) {
      DEBUG_MAP   yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   DEBUG_MAP   yLOG_char    ("a_axis"    , a_axis);
   rc = ymap_pick_map (a_axis, &x_map, &x_grid);
   --rce;  if (rc < 0) {
      DEBUG_MAP   yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   /*---(identity)-----------------------*/
   DEBUG_MAP   yLOG_note    ("self");
   if (a_full != '-') {
      x_map->axis  = a_axis;
      x_map->inc   = 1;
   }
   /*---(identity)-----------------------*/
   if (a_full == 'I') {
      DEBUG_MAP   yLOG_note    ("grid initialize");
      x_map->glen  = 0;
      ymap_grid_free (a_full, a_axis);
   } else if (a_full == 'y') {
      DEBUG_MAP   yLOG_note    ("grid removal");
      x_map->glen      = 0;
      ymap_grid_free (a_full, a_axis);
   } else {
      DEBUG_MAP   yLOG_note    ("grid cleaning");
      ymap_grid_clear (x_grid, x_map->glen);
   }
   /*---(lefts)--------------------------*/
   DEBUG_MAP   yLOG_note    ("mins");
   x_map->umin  = 0;
   x_map->gmin  = 0;
   x_map->gamin = 0;
   x_map->glmin = 0;
   x_map->gprev = 0;
   /*---(map)----------------------------*/
   DEBUG_MAP   yLOG_note    ("map");
   if (a_full == 'I') {
      x_map->mlen     = 0;
      x_map->map      = NULL;
   } else {
      x_map->mlen     = 0;
      if (x_map->map  != NULL) {
         DEBUG_MAP   yLOG_note    ("freeing map");
         /*> printf ("freeing map\n");                                                <*/
         free (x_map->map );
      }
      x_map->map      = NULL;
   }
   /*---(middles)------------------------*/
   DEBUG_MAP   yLOG_note    ("mids");
   x_map->gmid  = 0;
   x_map->gavg  = 0;
   /*---(rights)-------------------------*/
   DEBUG_MAP   yLOG_note    ("maxs");
   x_map->umax  = 0;
   x_map->gmax  = 0;
   x_map->gamax = 0;
   x_map->glmax = 0;
   x_map->gnext = 0;
   /*---(indexes)------------------------*/
   if (a_full != '-') {
      DEBUG_MAP   yLOG_note    ("screen");
      x_map->ubeg  = 0;
      x_map->ucur  = 0;
      x_map->uend  = 0;
      x_map->ulen  = 0;
      x_map->utend = 0;
   }
   if (a_full == 'I')  x_map->uavail = 0;
   /*---(grids)--------------------------*/
   if (a_full != '-') {
      DEBUG_MAP   yLOG_note    ("grid");
      x_map->gbeg  = 0;
      x_map->gcur  = 0;
      x_map->gend  = 0;
   }
   /*---(complete)-----------------------*/
   DEBUG_MAP   yLOG_exit    (__FUNCTION__);
   return 0;
}

char  ymap_mapinit (uchar a_axis)  { return ymap__clear ('I', a_axis); }
char  ymap_factory (uchar a_axis)  { return ymap__clear ('y', a_axis); }
char  yMAP_clear   (uchar a_axis)  { return ymap__clear ('-', a_axis); }

char
yMAP_entry              (uchar a_axis, ushort n, short a_ref, uchar a_wide, uchar a_used)
{
   /*---(locals)-----------+-----+-----+-*/
   char        rce         =  -10;
   char        rc          =    0;
   tyMAP      *x_map       = NULL;
   tGRID      *x_grid      = NULL;
   uchar      *x_valid     = "Ï·|";
   /*---(header)-------------------------*/
   DEBUG_MAP   yLOG_enter   (__FUNCTION__);
   /*---(defense)------------------------*/
   DEBUG_MAP   yLOG_char    ("a_axis"    , a_axis);
   rc = ymap_pick_map (a_axis, &x_map, &x_grid);
   DEBUG_MAP   yLOG_value   ("pickmap"   , rc);
   --rce;  if (rc < 0) {
      DEBUG_MAP    yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   DEBUG_MAP   yLOG_point   ("x_map"     , x_map);
   DEBUG_MAP   yLOG_point   ("x_grid"    , x_grid);
   /*---(check)--------------------------*/
   DEBUG_MAP   yLOG_value   ("n"         , n);
   DEBUG_MAP   yLOG_value   ("glen"      , x_map->glen);
   --rce;  if (n >= x_map->glen - 1) {
      DEBUG_MAP    yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   /*---(populate)-----------------------*/
   if (a_ref  > -32000) x_grid [n].ref  = a_ref;
   if (a_wide != 255)   x_grid [n].wide = a_wide;
   if (a_used != 255) {
      /*> DEBUG_MAP   yLOG_complex ("a_used"    , "%3d/%c, %s, %3d/%c", a_used, a_used, x_valid, x_valid [3], x_valid [3]);   <*/
      if (a_used == ' ')                                     a_used = '·';
      if (a_used == 'y')                                     a_used = 'Ï';
      if (a_used == 0 || strchr (x_valid, a_used) == NULL)   a_used = '·';
      x_grid [n].used = a_used;
   }
   DEBUG_MAP   yLOG_complex ("change"    , "%3d, %6d, %3d, %c", n, x_grid [n].ref, x_grid [n].wide, x_grid [n].used);
   /*---(complete)-----------------------*/
   DEBUG_MAP   yLOG_exit    (__FUNCTION__);
   return 0;
}

char
yMAP_size               (uchar a_axis, ushort a_len)
{
   /*---(locals)-----------+-----+-----+-*/
   char        rce         =  -10;
   char        rc          =    0;
   char        x_tries     =    0;
   tyMAP      *x_map       = NULL;
   tGRID      *x_grid      = NULL;
   tGRID      *x_new       = NULL;
   int         i           =    0;
   /*---(header)-------------------------*/
   DEBUG_MAP   yLOG_enter   (__FUNCTION__);
   /*---(defense)------------------------*/
   DEBUG_MAP   yLOG_char    ("a_axis"    , a_axis);
   rc = ymap_pick_map (a_axis, &x_map, &x_grid);
   DEBUG_MAP   yLOG_value   ("pickmap"   , rc);
   --rce;  if (rc < 0) {
      DEBUG_MAP    yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   /*---(increment length)---------------*/
   ++a_len;  /* add sentinel */
   /*---(check)--------------------------*/
   DEBUG_MAP   yLOG_point   ("x_map"     , x_map);
   DEBUG_MAP   yLOG_point   ("x_grid"    , x_grid);
   if (x_grid != NULL && x_map->glen == a_len) {
      rc = yMAP_clear   (a_axis);
      DEBUG_MAP   yLOG_note    ("already right length, just cleared");
      DEBUG_MAP   yLOG_exit    (__FUNCTION__);
      return 0;
   }
   DEBUG_MAP   yLOG_note    ("new grid size");
   /*---(clear)--------------------------*/
   rc = ymap_factory (a_axis);
   DEBUG_MAP   yLOG_value   ("factory"   , rc);
   --rce;  if (rc < 0) {
      DEBUG_MAP   yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   DEBUG_MAP   yLOG_value   ("a_len"     , a_len);
   if (a_len == 0) {
      DEBUG_MAP   yLOG_exit    (__FUNCTION__);
      return 0;
   }
   /*---(allocate grid)------------------*/
   DEBUG_MAP    yLOG_note    ("allocate");
   while (x_new == NULL && x_tries < 10)  {
      ++x_tries;
      x_new = (tGRID *) malloc (sizeof (tGRID) * a_len);
   }
   DEBUG_MAP    yLOG_value   ("x_tries"   , x_tries);
   DEBUG_MAP    yLOG_point   ("x_new"     , x_new);
   --rce;  if (x_new == NULL) {
      DEBUG_MAP    yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   /*---(clear new grid)-----------------*/
   ymap_grid_clear (x_new, a_len);
   /*---(save back)----------------------*/
   ymap_grid_set   (a_axis, x_map, x_new);
   DEBUG_MAP   yLOG_complex ("assign"    , "%c, b%-10p, x%-10p, y%-10p, z%-10p, t%-10p", a_axis, g_ugrid, g_xgrid, g_ygrid, g_zgrid, g_wgrid);
   x_map->glen    = a_len;
   /*---(show)---------------------------*/
   DEBUG_MAP    yLOG_value   ("glen"      , x_map->glen);
   for (i = 0; i < x_map->glen; ++i) {
      DEBUG_MAP   yLOG_complex ("auditing"  , "%3d, %6d, %2d, %c", i, x_new [i].ref, x_new [i].wide, x_new [i].used);
   }
   /*---(complete)-----------------------*/
   DEBUG_MAP   yLOG_exit    (__FUNCTION__);
   return 0;
}

char
yMAP_allsize            (ushort u, ushort x, ushort y, ushort z, ushort w)
{
   char        rc          =    0;
   DEBUG_MAP   yLOG_enter   (__FUNCTION__);
   if (rc >= 0)  rc = yMAP_size (YMAP_UNIV , u);
   if (rc >= 0)  rc = yMAP_size (YMAP_XAXIS, x);
   if (rc >= 0)  rc = yMAP_size (YMAP_YAXIS, y);
   if (rc >= 0)  rc = yMAP_size (YMAP_ZAXIS, z);
   if (rc >= 0)  rc = yMAP_size (YMAP_WHEN , w);
   DEBUG_MAP   yLOG_exit    (__FUNCTION__);
   return rc;
}

int
ymap_grid_walk          (tyMAP *a_map)
{
   /*---(locals)-----------+-----+-----+-*/
   char        rce         =  -10;
   char        rc          =    0;
   int         x_len       =    0;
   int         i           =    0;          /* grid index                     */
   int         x_lims      =    0;
   int         x_end       =    0;
   int         x_used      =    0;
   int         x_sum       =    0;
   /*---(header)-------------------------*/
   DEBUG_MAP   yLOG_enter   (__FUNCTION__);
   /*---(defense)------------------------*/
   DEBUG_MAP   yLOG_point   ("a_map"     , a_map);
   --rce;  if (a_map == NULL) {
      DEBUG_MAP    yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   DEBUG_MAP   yLOG_point   ("grid"      , a_map->grid);
   --rce;  if (a_map->grid == NULL) {
      DEBUG_MAP    yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   DEBUG_MAP    yLOG_value   ("glen"      , a_map->glen);
   /*---(temp values)--------------------*/
   a_map->umin  = 0;
   a_map->gmid  = 0;
   a_map->gavg  = 0;
   a_map->gamin = a_map->glmin = 65001;
   a_map->gamax = a_map->glmax = 0;
   /*---(run grid)-----------------------*/
   for (i = 0; i < a_map->glen; ++i) {
      DEBUG_MAP   yLOG_complex ("checking"  , "%2d, %6d, %2d, %2d", i, a_map->grid [i].ref, a_map->grid [i].wide, x_len);
      if (a_map->grid [i].ref == YMAP_EMPTY)  break;
      x_len += a_map->grid [i].wide;
      /*---(full min/max)----------------*/
      if (i == 0) {
         a_map->gmin  = i;
      }
      a_map->umax  = x_len - 1;
      a_map->gmax  = i;
      /*---(absolute min/max)------------*/
      if (a_map->grid [i].used == '|') {
         DEBUG_MAP    yLOG_value   ("absolute"  , i);
         if (a_map->gamin > 65000)  a_map->gamin = i;
         a_map->gamax = i;
         ++x_lims;
         x_end += i;
      }
      /*---(local min/max)---------------*/
      if (a_map->grid [i].used == G_CHAR_HUGEDOT) {
         DEBUG_MAP    yLOG_value   ("hugedot"   , i);
         if (a_map->gamin > 65000)  a_map->gamin = i;
         if (a_map->glmin > 65000)  a_map->glmin = i;
         a_map->gamax = i;
         a_map->glmax = i;
         ++x_used;
         x_sum += i;
      }
      /*---(done)------------------------*/
   }
   DEBUG_MAP    yLOG_value   ("x_len"     , x_len);
   /*---(clean-up)-----------------------*/
   if (x_used > 0)  a_map->gavg = x_sum / x_used;
   if (x_lims > 0)  a_map->gmid = x_end / x_lims;
   else             a_map->gmid = a_map->gavg;
   if (a_map->gamin > 65000)  a_map->gamin = a_map->gcur;
   if (a_map->glmin > 65000)  a_map->glmin = a_map->gcur;
   if (a_map->gamax == 0 && a_map->grid [0].used == G_CHAR_SPACE)  a_map->gamax = a_map->gcur;
   if (a_map->glmax == 0 && a_map->grid [0].used == G_CHAR_SPACE)  a_map->glmax = a_map->gcur;
   /*---(complete)-----------------------*/
   DEBUG_MAP   yLOG_exit    (__FUNCTION__);
   return x_len;
}

char
yMAP_update             (uchar a_axis)
{
   /*---(locals)-----------+-----+-----+-*/
   char        rce         =  -10;
   char        rc          =    0;
   tyMAP      *x_map       = NULL;
   tGRID      *x_grid      = NULL;
   char        x_tries     =    0;
   int         x_len       =    0;
   int         i           =    0;          /* grid index                     */
   int         n           =    0;          /* map index                      */
   int         c           =    0;          /* width counter                  */
   /*---(header)-------------------------*/
   DEBUG_MAP   yLOG_enter   (__FUNCTION__);
   /*---(defense)------------------------*/
   DEBUG_MAP   yLOG_char    ("a_axis"    , a_axis);
   rc = ymap_pick_map (a_axis, &x_map, &x_grid);
   --rce;  if (rc < 0) {
      DEBUG_MAP   yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   /*---(run grid)-----------------------*/
   DEBUG_MAP    yLOG_value   ("glen"      , x_map->glen);
   x_len = ymap_grid_walk (x_map);
   /*> x_map->gamin = x_map->glmin = 65001;                                                                           <* 
    *> for (i = 0; i < x_map->glen; ++i) {                                                                            <* 
    *>    DEBUG_MAP   yLOG_complex ("checking"  , "%2d, %6d, %2d, %2d", i, x_grid [i].ref, x_grid [i].wide, x_len);   <* 
    *>    if (x_grid [i].ref == YMAP_EMPTY)  break;                                                                   <* 
    *>    x_len += x_grid [i].wide;                                                                                   <* 
    *>    /+---(full min/max)----------------+/                                                                       <* 
    *>    if (i == 0) {                                                                                               <* 
    *>       x_map->umin  = i;                                                                                        <* 
    *>       x_map->gmin  = x_grid [i].ref;                                                                           <* 
    *>    }                                                                                                           <* 
    *>    x_map->umax  = x_len - 1;                                                                                   <* 
    *>    x_map->gmax  = x_grid [i].ref;                                                                              <* 
    *>    /+---(absolute min/max)------------+/                                                                       <* 
    *>    if (x_grid [i].used == G_CHAR_GROUP) {                                                                      <* 
    *>       if (x_map->gamin > 65000)  x_map->glmin = x_grid [i].ref;                                                <* 
    *>       x_map->gamax = x_grid [i].ref;                                                                           <* 
    *>    }                                                                                                           <* 
    *>    /+---(local min/max)---------------+/                                                                       <* 
    *>    if (x_grid [i].used == G_CHAR_HUGEDOT) {                                                                    <* 
    *>       if (x_map->gamin > 65000)  x_map->glmin = x_grid [i].ref;                                                <* 
    *>       if (x_map->glmin > 65000)  x_map->glmin = x_grid [i].ref;                                                <* 
    *>       x_map->gamax = x_grid [i].ref;                                                                           <* 
    *>       x_map->glmax = x_grid [i].ref;                                                                           <* 
    *>    }                                                                                                           <* 
    *>    /+---(done)------------------------+/                                                                       <* 
    *> }                                                                                                              <*/
   DEBUG_MAP    yLOG_value   ("x_len"     , x_len);
   /*---(free)-----------*/
   if (x_map->map  != NULL) {
      DEBUG_MAP    yLOG_note    ("freeing map");
      free (x_map->map);
   }
   x_map->map   = NULL;
   x_map->mlen  = x_len;
   /*---(call off)-------*/
   --rce;  if (x_len <= 0) {
      DEBUG_MAP    yLOG_note    ("empty grid, call off");
      DEBUG_MAP   yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   /*---(alloc)----------*/
   DEBUG_MAP    yLOG_note    ("allocating new map");
   x_tries = 0;
   while (x_map->map == NULL && x_tries < 10)  {
      ++x_tries;
      x_map->map = (short *) malloc (sizeof (short) * x_map->mlen);
   }
   DEBUG_MAP    yLOG_value   ("x_tries"   , x_tries);
   DEBUG_MAP    yLOG_point   ("map"       , x_map->map);
   --rce;  if (x_map->map == NULL) {
      DEBUG_MAP    yLOG_note    ("FAILED");
      DEBUG_MAP    yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   /*---(populate map)-------------------*/
   for (i = 0; i < x_map->glen; ++i) {
      if (x_grid [i].ref == YMAP_EMPTY)  break;
      /*> printf ("LOOP %3d\n", i);                                                   <*/
      x_grid [i].unit = n;
      while (c < x_grid [i].wide) {
         /*> printf (" assign %3d into %3d %3d\n", i, n, c);                          <*/
         x_map->map [n] = i;
         ++n;  ++c;
      }
      c = 0;
   }
   /*---(print map)----------------------*/
   /*> for (i = 0; i < x_map->mlen; ++i) {                                            <* 
    *>    printf ("%3d  %d\n", i, x_map->map [i]);                                    <* 
    *> }                                                                              <*/
   /*---(grid points)--------------------*/
   ymap_update_grid  (x_map);
   ymap_display      (x_map);
   /*---(complete)-----------------------*/
   DEBUG_MAP   yLOG_exit    (__FUNCTION__);
   return 0;
}

static uchar s_axis = '-';
static uchar s_unit = 0;

char
yMAP_start              (uchar a_axis, ushort a_len)
{
   char        rc          =    0;
   DEBUG_MAP   yLOG_enter   (__FUNCTION__);
   DEBUG_MAP   yLOG_complex ("args"      , "%c %d", a_axis, a_len);
   rc = yMAP_size (a_axis, a_len);
   if (rc >= 0) {
      s_axis = a_axis;
      s_unit = 0;
   }
   DEBUG_MAP   yLOG_exit    (__FUNCTION__);
   return rc;
}

char
yMAP_append             (short a_ref, uchar a_wide, uchar a_used)
{
   char        rc          =    0;
   DEBUG_MAP   yLOG_enter   (__FUNCTION__);
   DEBUG_MAP   yLOG_complex ("args"      , "%4d %3d %c", a_ref, a_wide, a_used);
   rc = yMAP_entry (s_axis, s_unit, a_ref, a_wide, a_used);
   if (rc >= 0)  ++s_unit;
   DEBUG_MAP   yLOG_exit    (__FUNCTION__);
   return rc;
}

char
yMAP_finish             (void)
{
   char        rc          =    0;
   DEBUG_MAP   yLOG_enter   (__FUNCTION__);
   rc = yMAP_update (s_axis);
   s_axis = '-';
   s_unit = 0;
   DEBUG_MAP   yLOG_exit    (__FUNCTION__);
   return rc;
}

char
yMAP_axis_avail         (uchar a_axis, ushort a_avail)
{
   /*---(locals)-----------+-----+-----+-*/
   char        rce         =  -10;
   char        rc          =    0;
   tyMAP      *x_map       = NULL;
   /*---(header)-------------------------*/
   DEBUG_MAP   yLOG_enter   (__FUNCTION__);
   /*---(defense)------------------------*/
   DEBUG_MAP   yLOG_char    ("a_axis"    , a_axis);
   rc = ymap_pick_map (a_axis, &x_map, NULL);
   --rce;  if (rc < 0) {
      DEBUG_MAP   yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   /*---(assign)-------------------------*/
   x_map->uavail = a_avail;
   /*---(complete)-----------------------*/
   DEBUG_MAP   yLOG_exit    (__FUNCTION__);
   return 0;
}

char
yMAP_axis_force         (uchar a_axis, ushort a_beg, ushort a_cur, ushort a_end)
{
   /*---(locals)-----------+-----+-----+-*/
   char        rce         =  -10;
   char        rc          =    0;
   tyMAP      *x_map       = NULL;
   /*---(header)-------------------------*/
   DEBUG_MAP   yLOG_enter   (__FUNCTION__);
   /*---(defense)------------------------*/
   DEBUG_MAP   yLOG_char    ("a_axis"    , a_axis);
   rc = ymap_pick_map (a_axis, &x_map, NULL);
   --rce;  if (rc < 0) {
      DEBUG_MAP   yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   /*---(assign)-------------------------*/
   x_map->gbeg  = a_beg;
   x_map->ubeg  = x_map->grid [x_map->gbeg].unit;
   x_map->gcur  = a_cur;
   x_map->ucur  = x_map->grid [x_map->gcur].unit;
   x_map->gend  = a_end;
   x_map->uend  = x_map->grid [x_map->gend].unit;
   /*---(rework display)-----------------*/
   ymap_display      (x_map);
   /*---(complete)-----------------------*/
   DEBUG_MAP   yLOG_exit    (__FUNCTION__);
   return 0;
}

char
yMAP_axis_grid          (uchar a_axis, ushort *a_beg, ushort *a_cur, ushort *a_end, uchar *a_extra)
{
   /*---(locals)-----------+-----+-----+-*/
   char        rce         =  -10;
   char        rc          =    0;
   tyMAP      *x_map       = NULL;
   /*---(header)-------------------------*/
   DEBUG_MAP   yLOG_enter   (__FUNCTION__);
   /*---(defense)------------------------*/
   DEBUG_MAP   yLOG_char    ("a_axis"    , a_axis);
   rc = ymap_pick_map (a_axis, &x_map, NULL);
   --rce;  if (rc < 0) {
      DEBUG_MAP   yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   /*---(assign)-------------------------*/
   if (a_beg   != NULL)  *a_beg   = x_map->gbeg;
   if (a_cur   != NULL)  *a_cur   = x_map->gcur;
   if (a_end   != NULL)  *a_end   = x_map->gend;
   if (a_extra != NULL)  *a_extra = x_map->uavail - x_map->ulen;
   /*---(complete)-----------------------*/
   DEBUG_MAP   yLOG_exit    (__FUNCTION__);
   return 0;
}


